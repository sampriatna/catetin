...
// --- 1) Double click / request ulang dengan idempotency_key sama ---
{
  ...
  ok(a.report.id === "dr_KSM_2026-08-01", "Report id kanonik per outlet+tanggal (bukan hash submission)");
...
// --- 2) Double click tanpa menunggu state (simulasi dua panggilan berurutan, key sama) ---
{
  ...
  const canon = canonicalLaporanCashTxId({ outlet: "KSM", date: "2026-08-02", businessId: state.businessId });
  ok(state.transactions.filter((t) => t.id === canon || t.id === `t_${a.report.id}_cash`).length === 1, "Mutate kedua tidak dobelkan tx");
...
// --- 4) Bug Kisamen: laporan hilang, user kirim lagi → dedupe cash ---
{
  ...
  // applyDailyReportMutation sudah reconcile — duplikat legacy harus hilang di sini
  ok(
    state.transactions.filter((t) => /laporan harian/i.test(t.source || "") && t.date === "2026-08-01").length === 1,
    "reconcile mendeteksi cash dobel"
  );
  ...
  const keepCanon = canonicalLaporanCashTxId(built.report) || `t_${built.report.id}_cash`;
  ok(
    fixed.transactions.some((t) => t.id === keepCanon || t.dailyReportId === built.report.id),
...
// --- 12) Samtaro 3 Agu: incomplete boleh rebuild; complete → UPSERT slot (bukan report #2) ---
{
  const state = emptyState();
  state.wallets.push({ id: "w_laci_smt", opening: LACI_FLOOR, floor: LACI_FLOOR });
  state.dailyReports.push({
  ...
  ok(okSubmit.report.id === "dr_SMT_2026-08-03", "SMT memakai id kanonik slot");

  const upsert = submitDailyReport(state, {
    channels: { tunai: 999, edc_bca: 0, gofood: 0 },
    date: "2026-08-03",
    user: { id: "u_smt", name: "Kasir Samtaro", outlet: "SMT" },
    submissionId: "biz1|SMT|2026-08-03|u_smt|n2",
  });
  ok(upsert.idempotent !== true && upsert.report.id === "dr_SMT_2026-08-03", "Nominal beda → UPSERT id sama");
  applyDailyReportMutation(state, upsert);
  ok(state.dailyReports.length === 1, "UPSERT tidak menambah laporan kedua");
  ok(state.dailyReports[0].setoranOwner === 999, "Nominal ter-update di slot yang sama");
