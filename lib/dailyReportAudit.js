...
export function auditDailyReportSlot(state, { businessId, outlet, date } = {}) {
  ...
  const guessSource = (r) => {
    if (!r) return "unknown";
    if (r.recoveredFromOrphan) return "orphan_recovery";
    const key = String(r.idempotencyKey || r.submissionId || "");
    if (key.endsWith("|omzet") || /\|SMT\|[^|]+\|omzet$/.test(key)) return "submit_stable_omzet_key";
    if (key.split("|").length >= 5) return "submit_legacy_nonce";
    if (r.syncStatus === "synced" || r.commitStatus === "committed") return "sync_bound";
    return "unknown";
  };
  ...
  const records = reports.map((r) => ({
    ...
    updatedAt: r.resubmittedAt || r.settledAt || r.adminVerifiedAt || r.submittedAt || null,
    outlet: r.outlet,
    reportDate: normalizeReportDate(r.date) || r.date,
    nominal: r.total ?? null,
    setoranTunai: r.setoranOwner ?? r.cash ?? null,
    status: r.status || null,
    commitStatus: r.commitStatus || null,
    syncStatus: r.syncStatus || null,
    serverRecordId: r.serverRecordId || r.id,
    idempotencyKey: r.idempotencyKey || r.submissionId || null,
    submissionId: r.submissionId || null,
    sourceGuess: guessSource(r),
    isPrimaryCandidate: canonical?.id === r.id,
  }));
  ...
  return {
    ...
    records,
    primaryRecordId: canonical?.id || null,
    primaryRecord: records.find((r) => r.isPrimaryCandidate) || null,
...
export function previewDuplicateCleanup(state, slotAudit) {
  ...
  const keepReportId = slotAudit.canonicalReportId || slotAudit.primaryRecordId;
  ...
  return {
    ...
    wouldDeleteTxIds,
    recommendation: [
      "1. Jangan DELETE mentah dari SQL tanpa backup app_state.",
      "2. Tetapkan record utama = primaryRecordId (status tertinggi / terbaru).",
      "3. Relink cash tx ke id kanonik t_cash_<OUTLET>_<DATE>; tombstone id cash loser.",
      "4. Collapse dailyReports ke 1 per slot (atau biarkan save merge finalizeMergedDoc).",
      "5. Verifikasi settle/transfer terkait tidak memakai reportId yang di-tombstone.",
      "6. Setelah cleanup, kasir cukup sync — jangan kirim ulang nominal.",
    ],
