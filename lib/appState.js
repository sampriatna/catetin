...
/** Tx tunai laporan aktif/settled tidak boleh masuk tombstone selama laporannya masih hidup. */
function filterProtectedLaporanTxDeletes(deletedIds, reports, transactions) {
  const cashBackedIds = new Set(
    (reports || []).filter((r) => CASH_BACKED_REPORT_STATUSES.has(r?.status)).map((r) => r.id)
  );
  const protectedIds = new Set();
  for (const r of reports || []) {
    if (CASH_BACKED_REPORT_STATUSES.has(r?.status)) {
  ...
  for (const t of transactions || []) {
    ...
    if (t.dailyReportId && cashBackedIds.has(t.dailyReportId)) protectedIds.add(t.id);
    if (t.sourceReportId && cashBackedIds.has(t.sourceReportId)) protectedIds.add(t.id);
  }
...
function finalizeMergedDoc(merged) {
  ...
  const pendingLaciFix = reconcilePendingLaciTransactions(doc);
  if (pendingLaciFix.changed) {
    doc = {
      ...doc,
      transactions: pendingLaciFix.transactions,
      deletedTransactionIds: pendingLaciFix.deletedTransactionIds,
    };
  }
...
export async function loadAppState(bizId) {
  ...
  // Load awal harus mendapat proteksi yang sama dengan jalur save/realtime.
  // Tanpa ini, laporan aktif dapat terlihat sementara tx tunai kanoniknya hilang
  // sampai ada perubahan awan berikutnya (kasus KBU 16 Agustus).
  return finalizeMergedDoc(merged);
}
