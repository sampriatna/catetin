...
/**
 * Cek kesiapan settle vs modal laci stabil (default Rp 250.000).
...
export function laciSettleCheck(state, report, { tolerance = LACI_SELISIH_TOLERANCE } = {}) {
  ...
  const floor = Math.round(Number(laciWallet?.floor ?? report?.laciFloor ?? LACI_FLOOR) || LACI_FLOOR);
  ...
  return {
    ...
    tolerance,
    message,
    revisionNoteSuggestion: !ok || reason !== "ok"
...
export function settleDailyReport(state, reportId, adminUser) {
  ...
  if (cashAmt > 0) {
    ...
    // Modal harus stabil di floor (250rb). Selisih → TOLAK settle, minta revisi kasir.
