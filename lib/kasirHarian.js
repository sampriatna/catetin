...
export function makeDailyReportSubmissionId({ businessId, outlet, date, userId, nonce } = {}) {
  const b = String(businessId || "biz").trim() || "biz";
...
export function isCompleteDailyReport(report, transactions = []) {
  ...
  if (cashAmt > 0) {
    const hasCash = (transactions || []).some((t) => {
      ...
      if (t.dailyReportId === report.id) return true;
...
export function submitDailyReport(state, payload) {
  ...
  const businessId = String(payload.businessId || state.businessId || "").trim() || null;
  ...
  // Idempotency: request yang sama + laporan lengkap → kembalikan tanpa tx baru
  ...
  if (prior && isCompleteDailyReport(prior, txsAll)) {
    ...
    return { report: prior, txs: existingTxs, removeIds: [], idempotent: true };
  }

  // Slot harus outlet+tanggal — laporan outlet lain di tanggal sama BUKAN duplicate
  const conflict = findDailyReportInSlot(reports, outlet, date);
  let removeIds = [];
  if (conflict) {
    // Prior incomplete dengan submissionId sama: ganti record rusak, jangan kunci user
  ...
  logDailyReportStage("submit_built", {
    ...
    reportDate: date,
    nominal: total,
    replacedIncomplete: removeIds.length > 0 || !!(conflict && !isCompleteDailyReport(conflict, txsAll)),
