...
test("patchWalletCatalog does not inject Go Food into NF Fishing wallets", () => {
  ...
  assert.ok(!wallets.some((w) => w.id === "w_tiktok_go"));
  assert.equal(wallets.length, 2);
});

test("patchWalletCatalog injects TikTok Go only into F&B catalog", () => {
  const wallets = patchWalletCatalog([
    { id: "w_laci_kbu", name: "Laci KBU", type: "kas_fisik", outlet: "KBU", sort: 10 },
    { id: "w_kas_besar", name: "Kas Besar", type: "kas_fisik", sort: 40 },
  ]);
  const tiktok = wallets.find((w) => w.id === "w_tiktok_go");
  assert.equal(tiktok?.name, "TikTok Go");
  assert.equal(tiktok?.type, "digital");
});
