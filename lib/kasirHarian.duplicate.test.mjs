...
// --- 5) Edit existing tanpa ubah (idempotent reject / same key) ---
{
  const state = emptyState();
  ...
  let rejected = false;
  try {
    submitDailyReport(state, smtPayload("2026-08-05", sid + "|other"));
  } catch (e) {
    rejected = /sudah dikirim/i.test(e.message);
  }
  ok(rejected, "5) edit/submit baru ditolak jika slot lengkap");
...
// --- 11) Tombstone same-ms resubmit tidak menelan laporan baru ---
{
  ...
  ok(filterDeletedDailyReports([{ ...neu, id: "dr_old" }], meta).length === 0, "11) id lama tetap diblok");
}
