...
export function buildTikTokGoSettlementMutation({
  ...
  const grossAmount = Math.round(Math.max(0, Number(sale.amount) || 0));
  const usingLegacyFees = totalDeduction === undefined || totalDeduction === null;
  const feeTotal = usingLegacyFees
    ? legacyFeeTotal(fees)
  ...
  const usingLegacyNet = reportedNetAmount === undefined || reportedNetAmount === null;
  const reportedNet = usingLegacyNet
    ? expectedNetAmount
  ...
  const usingLegacyBankAmount = actualBankAmount === undefined || actualBankAmount === null;
  const bankAmount = usingLegacyBankAmount
    ? reportedNet
