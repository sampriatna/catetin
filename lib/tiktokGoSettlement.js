...
export function computeTikTokGoSummary({ transactions = [], wallets = [] } = {}) {
  ...
  return {
    ...
    totalSettled: rows.reduce((sum, row) => sum + (row.settlement ? Number(row.settlement.amount) || 0 : 0), 0),
...
function normalizeFees(fees = {}) {
  const out = {};
  TIKTOK_GO_FEE_TYPES.forEach(({ id }) => {
    out[id] = Math.round(Math.max(0, Number(fees[id]) || 0));
  });
  return out;
}
...
export function buildTikTokGoSettlementMutation({
  ...
  if (existing) return { updatedSale: sale, transactions: [], idempotent: true };
  ...
  if (!proof) throw new Error("Bukti penerimaan / nomor mutasi bank wajib diisi.");
  const destination = String(destinationWalletId || "").trim();
  if (!destination) throw new Error("Rekening tujuan settlement wajib dipilih.");

  const normalizedFees = normalizeFees(fees);
  const feeTotal = Object.values(normalizedFees).reduce((sum, amount) => sum + amount, 0);
  const grossAmount = Math.round(Math.max(0, Number(sale.amount) || 0));
  if (feeTotal >= grossAmount) {
    throw new Error("Total potongan harus lebih kecil dari penjualan bruto.");
  }
  const netSettlementAmount = grossAmount - feeTotal;
  const actionId = `tiktok_go_settlement_${sale.id}`;
  const feeTransactions = TIKTOK_GO_FEE_TYPES
    .filter(({ id }) => normalizedFees[id] > 0)
    .map(({ id, label }) => ({
      id: `t_${sale.id}_fee_${id}`,
      type: "out",
      amount: normalizedFees[id],
      categoryId: TIKTOK_GO_FEE_CATEGORY_ID,
      walletId: TIKTOK_GO_WALLET_ID,
      desc: `${label} TikTok Go · ${sale.meta?.outlet || sale.date || ""}`.trim(),
      date: actualDate,
      source: "Potongan TikTok Go",
      transactionKind: "tiktok_go_fee",
      idempotencyKey: `${actionId}|fee|${id}`,
      meta: {
        tiktokGoKind: "fee",
        tiktokGoFeeType: id,
  ...
  return {
    updatedSale: {
      ...
      meta: {
        ...
        actualSettlementDate: actualDate,
        destinationWalletId: destination,
        destinationLabel: destination === TIKTOK_GO_DESTINATION_WALLET_ID ? "Rudi Mandiri" : null,
        evidenceRef: proof,
        feeBreakdown: normalizedFees,
        feeTotal,
        netSettlementAmount,
        settlementTransactionId: settlement.id,
      },
    },
    transactions: [...feeTransactions, settlement],
    idempotent: false,
  };
}
