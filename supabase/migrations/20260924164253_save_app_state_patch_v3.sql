-- Save only changed top-level app_state fields plus changed transaction rows.
-- Keeps the existing optimistic concurrency check on app_state.updated_at.
...
begin
  if p_expected_updated_at is null then
    ...
    values (p_business_id, coalesce(p_patch, '{}'::jsonb), v_updated_at)
    on conflict (business_id) do nothing
    returning updated_at into v_result;
  else
    update public.app_state
    set data = ((coalesce(data, '{}'::jsonb) - coalesce(p_remove_keys, '{}'::text[])) || coalesce(p_patch, '{}'::jsonb)),
