-- ============================================================================
-- DRAFT 06 — RLS + quantity/valuation RPCs (no cost leak)
-- STATUS: JANGAN DIJALANKAN sampai Owner approve
-- Prerequisite: 01–05
--
-- Outlet TIDAK punya policy SELECT pada tabel ber-cost.
-- Quantity hanya lewat RPC SECURITY DEFINER yang sudah filter lokasi.
-- ============================================================================
...
-- Master tanpa cost: reader boleh
...
create policy inv_loc_select on public.inventory_locations for select
  using (public.is_inventory_reader(business_id) and (
...
create policy inv_loc_write on public.inventory_locations for all
  using (public.business_role(business_id) = 'owner' or public.has_permission(business_id, 'manage_items', null))
  with check (public.business_role(business_id) = 'owner' or public.has_permission(business_id, 'manage_items', null));
...
-- COST TABLES: HANYA valuer (owner/admin/forecasting)
-- inventory_lots, stock_movements, stock_balances, stock_lot_balances,
-- stock_transfer_lines, waste_records, inventory_receipt_lines (unit_cost),
-- stock_opname_lines (difference_value)
...
-- Tidak ada INSERT/UPDATE/DELETE policy ledger untuk authenticated
-- (revoke sudah di 03; tanpa policy = deny under RLS)
...
-- Transfer header: outlet tujuan/sumber boleh lihat (tanpa lines ber-cost)
...
-- Receipt header partial (tanpa cost di header) — outlet receiver
drop policy if exists transfer_receipts_select on public.stock_transfer_receipts;
create policy transfer_receipts_select on public.stock_transfer_receipts for select
  using (
    public.is_inventory_valuer(business_id)
    or exists (
...
create policy transfer_receipt_lines_select on public.stock_transfer_receipt_lines for select
  using (
    public.is_inventory_valuer(business_id)
    or exists (
      select 1
      from public.stock_transfer_receipts r
...
create policy opnames_select on public.stock_opnames for select
  using (public.is_inventory_valuer(business_id) or public.can_access_location(location_id));

-- Settings: reader lihat; write owner
...
drop policy if exists biz_state_write on public.inventory_business_state;
create policy biz_state_write on public.inventory_business_state for all
  using (public.business_role(business_id) = 'owner')
  with check (public.business_role(business_id) = 'owner');
...
-- purchasing_tx_links: purchasing create; admin/owner review (write policies ketat di app/RPC)
drop policy if exists ptx_select on public.purchasing_tx_links;
create policy ptx_select on public.purchasing_tx_links for select
  using (
    public.is_inventory_valuer(business_id)
    or public.business_role(business_id) = 'purchasing'
  );
...
create policy ptx_insert_purchasing on public.purchasing_tx_links for insert
...
create policy ptx_update_review on public.purchasing_tx_links for update
  using (
    public.business_role(business_id) = 'owner'
    or (
      public.business_role(business_id) = 'admin'
      and public.has_permission(business_id, 'review_purchasing_link', null)
    )
    or (
      public.business_role(business_id) = 'purchasing'
      and review_status in ('none', 'pending_review')
      and locked = false
    )
  )
  with check (
...
-- Receipts header: valuer / forecasting
...
-- ============================================================================
-- Quantity RPCs (SECURITY DEFINER) — grant ke authenticated
-- ============================================================================
...
begin
  ...
  if not public.is_inventory_reader(p_business_id) then
    raise exception 'forbidden';
  end if;

  return query
  select
    b.item_id, b.location_id, i.sku, i.name, i.stock_unit, loc.code,
...
begin
  ...
  return query
  select
    lb.lot_id, l.item_id, lb.location_id, l.batch_number,
    l.expiry_date, l.expiry_unknown, lb.quantity
  ...
  -- TIDAK mengembalikan initial_unit_cost
end;
$$;
...
begin
  ...
  if not (
    public.is_inventory_valuer(p_business_id)
    or public.can_access_location(p_location_id)
  ) then
...
begin
  if auth.uid() is null then raise exception 'unauthorized'; end if;
  if not public.is_inventory_valuer(p_business_id) then
    raise exception 'forbidden: valuation';
  end if;
...
-- Transfer lines quantity-only untuk receiver outlet
...
begin
  ...
  return query
  select
    l.id, l.item_id, l.lot_id, i.sku, i.name,
    l.sent_qty, l.received_qty_total, l.batch_code, l.expiry_date
  from public.stock_transfer_lines l
  join public.inventory_items i on i.id = l.item_id
  where l.transfer_id = p_transfer_id;
  -- TIDAK mengembalikan unit_cost
end;
$$;
