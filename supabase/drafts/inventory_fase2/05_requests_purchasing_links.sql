-- ============================================================================
-- DRAFT 05 — Requests / PR / PO / purchasing_tx_links (revisi putaran 3)
...
create table if not exists public.inventory_requests (
  ...
  status text not null default 'draft' check (status in (
    'draft','submitted','checking_stock','fulfilled_from_warehouse',
    'transfer_created','needs_purchase','completed','rejected'
  )),
  notes text null,
...
create unique index if not exists uq_inventory_requests_id_biz on public.inventory_requests (id, business_id);
drop trigger if exists trg_inventory_requests_updated on public.inventory_requests;
create trigger trg_inventory_requests_updated before update on public.inventory_requests
...
create table if not exists public.inventory_request_lines (
  ...
  reason text not null check (reason in (
    'min_stock','forecast','event','reservation','damaged','expired','sudden_increase','manual'
  )),
  notes text null,
...
create table if not exists public.purchase_requests (
  ...
  status text not null default 'draft' check (status in (
    'draft','submitted','approved','rejected','partially_ordered','ordered','closed','cancelled'
  )),
  requested_by uuid null references public.profiles(id) on delete set null,
  ...
  foreign key (inventory_request_id, business_id)
    references public.inventory_requests (id, business_id) on delete restrict
);
create unique index if not exists uq_purchase_requests_id_biz on public.purchase_requests (id, business_id);
drop trigger if exists trg_purchase_requests_updated on public.purchase_requests;
create trigger trg_purchase_requests_updated before update on public.purchase_requests
  for each row execute function public.inventory_set_updated_at();
...
create table if not exists public.purchase_request_lines (
  ...
  foreign key (inventory_request_line_id, business_id)
    references public.inventory_request_lines (id, business_id) on delete restrict
);
...
create table if not exists public.purchase_orders (
  ...
  business_id uuid not null references public.businesses(id) on delete cascade,
  po_no text not null,
  -- from_po path: purchase_request_id WAJIB (ditegakkan di link + trigger)
...
create unique index if not exists uq_purchase_orders_id_biz on public.purchase_orders (id, business_id);
...
declare v_line_pr uuid; v_po_pr uuid;
begin
  if tg_table_name = 'purchase_order_lines' and new.purchase_request_line_id is not null then
    ...
    if v_line_pr is null then raise exception 'purchase_request_line tidak ditemukan'; end if;
    if v_po_pr is null then raise exception 'PO wajib mempunyai purchase_request_id'; end if;
    if v_line_pr is distinct from v_po_pr then
...
create trigger trg_po_lines_parent before insert or update on public.purchase_order_lines
...
create table if not exists public.purchasing_tx_links (
  ...
  purchase_kind text not null check (purchase_kind in ('from_pr','from_po','direct','emergency')),
  purchase_request_id uuid null,
  purchase_request_line_id uuid null,
  ...
  constraint purchasing_tx_links_kind_check check (
    ...
    or (purchase_kind = 'from_po'
      and purchase_request_id is not null
      and purchase_order_id is not null
      and direct_reason is null)
    or (purchase_kind in ('direct','emergency')
...
create trigger trg_purchasing_tx_links_updated before update on public.purchasing_tx_links
  for each row execute function public.inventory_set_updated_at();

create or replace function public.purchasing_tx_links_validate()
returns trigger language plpgsql as $$
declare v_pr uuid; v_po uuid; v_po_pr uuid;
begin
  ...
  if new.purchase_request_line_id is not null then
    select purchase_request_id into v_pr from public.purchase_request_lines
    where id = new.purchase_request_line_id and business_id = new.business_id;
  ...
  if new.purchase_order_line_id is not null then
    select purchase_order_id into v_po from public.purchase_order_lines
  ...
  if new.purchase_kind = 'from_po' then
    select purchase_request_id into v_po_pr from public.purchase_orders
    where id = new.purchase_order_id and business_id = new.business_id;
    if v_po_pr is null then
      raise exception 'from_po: PO.purchase_request_id wajib terisi';
    end if;
    if v_po_pr is distinct from new.purchase_request_id then
      raise exception 'from_po: PO.purchase_request_id harus sama dengan link.purchase_request_id';
    end if;
  end if;
  ...
  if tg_op = 'UPDATE' then
    ...
    -- Bekukan SEMUA field penting setelah approved/locked/received
    if old.locked or old.review_status = 'approved' or old.received then
      if new.app_tx_id is distinct from old.app_tx_id
         ...
         or new.purchase_request_line_id is distinct from old.purchase_request_line_id
         or new.purchase_order_id is distinct from old.purchase_order_id
         or new.purchase_order_line_id is distinct from old.purchase_order_line_id
         or new.direct_reason is distinct from old.direct_reason
         or new.is_active is distinct from old.is_active
         or (
              -- review fields hanya boleh diubah oleh RPC review (flag)
              coalesce(current_setting('inventory.purchasing_review_rpc', true), '') <> '1'
              and (
         ...
         or (
              coalesce(current_setting('inventory.purchasing_receive_rpc', true), '') <> '1'
              and new.received is distinct from old.received
            )
         or (
              coalesce(current_setting('inventory.purchasing_lock_rpc', true), '') <> '1'
              and new.locked is distinct from old.locked
            )
      then
        raise exception 'link approved/locked/received tidak boleh diedit';
      end if;
    end if;
  end if;
  return new;
...
begin
  if old.locked or old.review_status = 'approved' or old.received then
    raise exception 'link approved/locked/received tidak boleh dihapus';
  end if;
  return old;
end;
$$;
...
create trigger trg_purchasing_tx_links_del before delete on public.purchasing_tx_links
...
alter table public.inventory_receipts
  ...
  references public.purchasing_tx_links (id, business_id) on delete restrict;
...
language sql stable security definer set search_path = public as $$
  with tx_ids as (
    ...
    where s.business_id = p_business_id and coalesce(elem->>'id','') <> ''
  )
  select l.id, l.app_tx_id, 'orphan_app_tx_missing'::text
  from public.purchasing_tx_links l
  where l.business_id = p_business_id and l.is_active
    and not exists (select 1 from tx_ids t where t.tx_id = l.app_tx_id);
$$;
revoke all on function public.validate_purchasing_tx_link_orphans(uuid)
...
create trigger trg_purchasing_transactions_updated before update on public.purchasing_transactions
  for each row execute function public.inventory_set_updated_at();
